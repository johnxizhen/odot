...
describe UserSessionsController do
  ...
  describe "POST 'create'" do
    context "with correct credentials" do
      let!(:user) { User.create(first_name: "John", last_name: "Li", email: "johnli@ecenglish.com", password: "administrator", password_confirmation: "administrator") }
      it "redirects to the todo list path" do
        post :create, email: "johnli@ecenglish.com", password: "administrator"
        expect(response).to be_redirect
        expect(response).to redirect_to(todo_lists_path)
      end

      it "finds the user" do
        expect(User).to receive(:find_by).with({email: "johnli@ecenglish.com"}).and_return(user)
        post :create, email: "johnli@ecenglish.com", password: "administrator"
      end
      ...
      it "sets the remember_me_token cookie if chosen" do
        ...
        post :create, email: "johnli@ecenglish.com", password: "administrator", remember_me: "1"
    ...
    context "with no email in existence" do
      let(:email) { "nowhereland@example.com" }
      let(:password) { "incorrect" }
      it_behaves_like "denied login"
    end
  end
